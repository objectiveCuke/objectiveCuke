...
-(void)be:(SEL)sel {
	...
	BOOL result = (NSInteger)[value performSelector:sel];
	...
	for (NSString *key in selectors) {
		if (![key isEqualToString:@""]) {
			...
			if ([returnType isEqualToString:@"@"]) {
				...
				} else if (![[value performSelector:selector] isEqual:(id)expected]) {
					[errors addObject:[NSString	stringWithFormat:@"%@ : %@ is not equal to %@", key, [value performSelector:selector], expected]];
					foundErrors = YES;
					continue;
				}
			} else if ([value performSelector:selector] != (id)expected) {
...
//	NSMutableString *selector = [NSStringFromSelector(aSelector) mutableCopy];
	if (isBe) {
